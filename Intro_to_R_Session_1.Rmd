...
#if you are more familar with the arguments of the function
...
#if you are calculating the area of a triange with base length 2 cm and height 3 cm, you can go about it like this:
...
#let's repeat the example with the loan from the bank for the anti-misinformation platorm, this time without hard coding below is the text of the problem again:
...
#Task 2
#In the Eurpopean Social Survey (ESS), the left-right political ideology scale is from 0 to 10, 0 being extreme left and 10 being extreme right
...
#Task 3
#In the European Social Survey, political interest is scaled from 1-4, where 1 means very intested in politicas, and 4 means very uninterested in politics. This is counter intuitive. create a vector fot the political interest values of the following survey participants **then** reverse the scale so 4 would mean very interested in politics
...
#we will explore the true potential of this after some excersies
...
#you have a vector of ISO names of countris 
...
#the diference is clearer using the dim()
...
dim(matrix_example) #the dimensions we definied
...
print(df_example$country)
print(df_example[,1]) #remmeber [row,column]
...
#the condidion after if between the ( ) is met, that is why following code between the { } is executed
