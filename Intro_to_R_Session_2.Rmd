...
The are two ways excute successive commands on the same object: 1) the Base R method is to place the command to be executed first in the center then add other commands on top of it, like an onion.
...
Let's say we want to investigate if respondents (dis)trusting scientist are more likely to be willing to get vaccined against COVID-19.
...
#there is no easy way to reorder the column and it has to be done manually

#either change the order of names of indices
...
**This only works if there is a similiar data structure, so know your data !**
...
The **KOF Globalization Index** isone of the most prominent Globalization Indicies, and highly influential.
It measures Globalization on different dimensions and differantiates between *de facto* and *de jure* Globalization.
